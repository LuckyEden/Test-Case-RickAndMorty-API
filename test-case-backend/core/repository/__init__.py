...
class BaseRepository(Generic[ModelType]):
    ...
    async def create(self, attributes: dict[str, Any] = None) -> ModelType:
        ...
        for key in attributes:
            if key not in self.model_class.__table__.columns:
                raise KeyError(f"{key} is not a valid column for {self.model_class.__tablename__}")
    ...
    async def count(self, join_: set[str] | None = None) -> int:
        """
    ...
    async def get_random(self, count: int = 1, join_: set[str] | None = None) -> list[ModelType]:
        query = self._query(join_)
    ...
    async def _first_unique(self, query: Select) -> ModelType | None:
        result = await self.session.execute(query)
